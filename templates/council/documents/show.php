<?php
use Cruinn\Controllers\CouncilController;
?>
<div class="council-document-detail">
    <div class="page-header">
        <div>
            <a href="/council/documents" class="back-link">&larr; All Documents</a>
    ...
    <div class="document-card">
        <div class="document-meta-grid">
            ...
            <div class="meta-item">
                ...
                <span><?= CouncilController::formatFileSize((int)$document['file_size']) ?></span>
            </div>
            <div class="meta-item">
        ...
        <div class="document-actions">
            <a href="/council/documents/<?= (int)$document['id'] ?>/download" class="btn btn-primary">Download Current Version</a>

            <?php if ($document['status'] === 'draft'): ?>
                <form method="post" action="/council/documents/<?= (int)$document['id'] ?>/submit" class="inline-form">
                    <?= csrf_field() ?>
            ...
            <?php if ($document['status'] === 'submitted'): ?>
                <form method="post" action="/council/documents/<?= (int)$document['id'] ?>/approve" class="inline-form">
                    <?= csrf_field() ?>
            ...
            <?php if ($document['status'] !== 'archived'): ?>
                <form method="post" action="/council/documents/<?= (int)$document['id'] ?>/archive" class="inline-form">
                    <?= csrf_field() ?>
    ...
    <section class="document-section">
        ...
        <form method="post" action="/council/documents/<?= (int)$document['id'] ?>/version" enctype="multipart/form-data" class="version-upload-form">
            <?= csrf_field() ?>
            <div class="form-row">
        ...
        <table class="admin-table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><?= CouncilController::formatFileSize((int)$v['file_size']) ?></td>
                    <td><?= e($v['notes'] ?? '—') ?></td>
                    <td><time datetime="<?= e($v['created_at']) ?>"><?= format_date($v['created_at'], 'j M Y H:i') ?></time></td>
                    <td>
                        <a href="/council/documents/<?= (int)$document['id'] ?>/versions/<?= (int)$v['id'] ?>/download" class="btn btn-sm btn-secondary">Download</a>
                    </td>
                </tr>
                <?php endforeach; ?>
            </tbody>
    ...
    <section class="danger-zone">
        ...
        <form method="post" action="/council/documents/<?= (int)$document['id'] ?>/delete" onsubmit="return confirm('Delete this document permanently?')">
            <?= csrf_field() ?>
