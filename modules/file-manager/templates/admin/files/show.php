...
use IGA\Services\DocumentService;
...
$statusTransitions = [
    'draft' => [['status' => 'pending_review', 'label' => 'Submit for Review', 'class' => 'btn-primary']],
    'pending_review' => [
        ['status' => 'approved', 'label' => 'Approve', 'class' => 'btn-success'],
        ['status' => 'draft', 'label' => 'Return to Draft', 'class' => 'btn-secondary'],
    ],
    'approved' => [
        ['status' => 'published', 'label' => 'Mark Published', 'class' => 'btn-primary'],
        ['status' => 'archived', 'label' => 'Archive', 'class' => 'btn-secondary'],
    ],
    'published' => [
        ['status' => 'archived', 'label' => 'Archive', 'class' => 'btn-secondary'],
    ],
    'archived' => [
        ['status' => 'draft', 'label' => 'Restore to Draft', 'class' => 'btn-secondary'],
    ],
];
...
<div class="admin-page">
    ...
    <div class="admin-page-header">
        <div>
            ...
            <?php if ($file['folder_name']): ?>
                <p class="text-muted">in <a href="/files?folder=<?= (int)$file['folder_id'] ?>"><?= e($file['folder_name']) ?></a></p>
            <?php endif; ?>
        </div>
        <div class="admin-page-actions">
            <a href="/files<?= $file['folder_id'] ? '?folder=' . (int)$file['folder_id'] : '' ?>" class="btn btn-secondary">← Back</a>
        </div>
    </div>
    ...
    <div class="fm-actions">
        <?php if ($file['file_path']): ?>
            <a href="/files/<?= (int)$file['id'] ?>/download" class="btn btn-primary">⬇ Download</a>
        <?php endif; ?>

        <?php if ($canEdit && ($file['content_type'] === 'composed' || $file['parsed_content'])): ?>
            <a href="/files/<?= (int)$file['id'] ?>/edit" class="btn btn-secondary">✏ Edit Content</a>
        <?php endif; ?>

        <?php if ($file['parsed_content']): ?>
            <div class="btn-group">
                <form method="post" action="/files/<?= (int)$file['id'] ?>/export" class="inline-form">
                    <?= csrf_field() ?>
                ...
                <form method="post" action="/files/<?= (int)$file['id'] ?>/export" class="inline-form">
                    ...
                    <button type="submit" class="btn btn-secondary">Export DOCX</button>
                </form>
            </div>
        <?php endif; ?>

        <!-- Status transitions -->
        <?php foreach ($statusTransitions[$file['status']] ?? [] as $transition): ?>
            <form method="post" action="/files/<?= (int)$file['id'] ?>/status" class="inline-form">
                <?= csrf_field() ?>
                <input type="hidden" name="status" value="<?= e($transition['status']) ?>">
                <button type="submit" class="btn <?= e($transition['class']) ?>"><?= e($transition['label']) ?></button>
            </form>
        <?php endforeach; ?>

        <?php if ($canEdit): ?>
            <form method="post" action="/files/<?= (int)$file['id'] ?>/delete" class="inline-form" onsubmit="return confirm('Permanently delete this file and all versions?')">
                <?= csrf_field() ?>
    ...
    <div class="fm-tabs">
        ...
        <button class="fm-tab" onclick="switchTab(this, 'publish')">Publish (<?= count($publications) ?>)</button>
        ...
        <?php elseif ($file['file_path']): ?>
            ...
            <?php else: ?>
                <div class="fm-no-preview">
                    ...
                    <a href="/files/<?= (int)$file['id'] ?>/download" class="btn btn-primary">Download to View</a>
                </div>
            <?php endif; ?>
        <?php else: ?>
            <div class="fm-no-preview">
                ...
                <?php if ($canEdit): ?>
                    <a href="/files/<?= (int)$file['id'] ?>/edit" class="btn btn-primary">Start Writing</a>
                <?php endif; ?>
            </div>
        <?php endif; ?>
    </div>
    ...
    <div class="fm-tab-content" id="tab-sharing" style="display:none">
        <?php if ($canEdit): ?>
            <form method="post" action="/files/<?= (int)$file['id'] ?>/share" class="fm-share-form">
                <?= csrf_field() ?>
                <div class="form-row">
                ...
                <tbody>
                    <?php foreach ($shares as $share): ?>
                        <tr>
                            ...
                            <?php if ($canEdit): ?>
                                <td>
                                    <form method="post" action="/files/<?= (int)$file['id'] ?>/unshare" class="inline-form">
                                        <?= csrf_field() ?>
    ...
    <!-- Tab: Publish -->
    <div class="fm-tab-content" id="tab-publish" style="display:none">
        <?php if ($file['parsed_content'] && in_array($file['status'], ['approved', 'published']) || ($current_user['role'] ?? '') === 'admin'): ?>
            <form method="post" action="/files/<?= (int)$file['id'] ?>/publish" class="fm-publish-form">
                <?= csrf_field() ?>
                <h4>Publish To</h4>
                <div class="fm-publish-targets">
                    <label class="fm-publish-target">
                        <input type="checkbox" name="targets[]" value="article">
                        <span class="fm-publish-target-card">
                            <span class="fm-publish-icon">📰</span>
                            <span class="fm-publish-name">News Article</span>
                            <span class="fm-publish-desc">Create a draft article from this document</span>
                        </span>
                    </label>
                    <label class="fm-publish-target">
                        <input type="checkbox" name="targets[]" value="event">
                        <span class="fm-publish-target-card">
                            <span class="fm-publish-icon">📅</span>
                            <span class="fm-publish-name">Event</span>
                            <span class="fm-publish-desc">Create a draft event from this document</span>
                        </span>
                    </label>
                    <label class="fm-publish-target">
                        <input type="checkbox" name="targets[]" value="mailing_list">
                        <span class="fm-publish-target-card">
                            <span class="fm-publish-icon">📧</span>
                            <span class="fm-publish-name">Mailing List</span>
                            <span class="fm-publish-desc">Queue for email distribution to members</span>
                        </span>
                    </label>
                    <label class="fm-publish-target">
                        <input type="checkbox" name="targets[]" value="social">
                        <span class="fm-publish-target-card">
                            <span class="fm-publish-icon">📱</span>
                            <span class="fm-publish-name">Social Media</span>
                            <span class="fm-publish-desc">Queue for social media posting</span>
                        </span>
                    </label>
                </div>
                <button type="submit" class="btn btn-primary">Publish Selected</button>
            </form>
        <?php elseif (!$file['parsed_content']): ?>
            <p class="text-muted">No publishable content. Upload a parseable document or compose content first.</p>
        <?php else: ?>
            <p class="text-muted">File must be approved before publishing. Current status: <strong><?= e($statusLabels[$file['status']] ?? $file['status']) ?></strong></p>
        <?php endif; ?>
            ...
            <table class="admin-table">
                ...
                <tbody>
                    <?php foreach ($publications as $pub): ?>
                        <tr>
                            <td>
                                <span class="badge"><?= e(ucfirst(str_replace('_', ' ', $pub['target_type']))) ?></span>
                                <?php if ($pub['target_id']): ?>
                                    <?php if ($pub['target_type'] === 'article'): ?>
                                        <a href="/admin/articles/<?= (int)$pub['target_id'] ?>/edit">View →</a>
                                    <?php elseif ($pub['target_type'] === 'event'): ?>
                                        <a href="/admin/events/<?= (int)$pub['target_id'] ?>/edit">View →</a>
                                    <?php endif; ?>
                                <?php endif; ?>
                            </td>
                            <td><?= e($pub['publisher_name'] ?? '—') ?></td>
                            <td><time><?= format_date($pub['published_at'], 'j M Y H:i') ?></time></td>
                            <td class="text-muted"><?= e($pub['notes'] ?? '—') ?></td>
                        </tr>
                    <?php endforeach; ?>
                </tbody>
            </table>
    ...
    <div class="fm-tab-content" id="tab-upload-version" style="display:none">
        <?php if ($canEdit): ?>
            <form method="post" action="/files/<?= (int)$file['id'] ?>/version" enctype="multipart/form-data" class="form-card">
                <?= csrf_field() ?>
                <div class="form-group">
