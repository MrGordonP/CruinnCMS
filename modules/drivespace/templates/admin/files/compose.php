<?php
/**
 * Drivespace — Document Composer / Editor
 *
...
$isEdit = !empty($file);
$formAction = $isEdit ? '/drivespace/' . (int)$file['id'] . '/edit' : '/drivespace/compose';
?>
<div class="admin-page">
    <div class="admin-page-header">
        <h1><?= $isEdit ? 'Edit Document' : 'New Document' ?></h1>
        <a href="<?= $isEdit ? '/drivespace/' . (int)$file['id'] : '/drivespace' ?>" class="btn btn-secondary">← Back</a>
    </div>
    ...
    <form method="post" action="<?= $formAction ?>" class="fm-compose-form" id="compose-form">
        ...
        <div class="fm-compose-footer">
            ...
            <a href="<?= $isEdit ? '/drivespace/' . (int)$file['id'] : '/drivespace' ?>" class="btn btn-secondary">Cancel</a>
            <?php if ($isEdit): ?>
...
(function() {
    ...
    const CSRF_TOKEN  = <?= json_encode(\Cruinn\CSRF::getToken()) ?>;
    ...
    function autosaveToServer() {
        ...
        fetch('/drivespace/' + FILE_ID + '/autosave', {
            method: 'POST',
            headers: {
