...
<form method="post" action="<?= url('/admin/templates/' . (int)$tpl['id']) ?>">
    ...
    <div class="form-row" style="align-items:flex-start;gap:2rem">
        ...
        <div style="flex:1;min-width:0">
            ...
            <h3 style="margin-top:1.5rem">Zone Canvas Assignments</h3>
            <p class="form-help" style="margin-top:0.5rem;margin-bottom:1rem">
                Assign reusable zone canvases to each zone slot in this template. The <code>main</code> zone always contains page-specific content and cannot be assigned.
            </p>

            <?php
            // Group zone canvases by zone_name for easier selection
            $canvasesByZone = [];
            foreach ($zoneCanvases ?? [] as $zc) {
                $zn = $zc['zone_name'] ?? '';
                if ($zn) {
                    $canvasesByZone[$zn][] = $zc;
                }
            }

            foreach ($zones as $zoneName):
                if ($zoneName === 'main') continue; // main is always page content
                $assignedCanvasId = $currentZoneCanvases[$zoneName] ?? null;
                $assignedCanvas = null;
                if ($assignedCanvasId) {
                    foreach ($zoneCanvases ?? [] as $zc) {
                        if ((int)$zc['id'] === (int)$assignedCanvasId) {
                            $assignedCanvas = $zc;
                            break;
                        }
                    }
                }
                $zoneLabel = ucfirst(str_replace(['-', '_'], ' ', $zoneName));
            ?>
            <div class="form-group" style="margin-bottom:1rem">
                <label for="zone_canvas_<?= e($zoneName) ?>">
                    <code><?= e($zoneName) ?></code> zone
                </label>
                <div style="display:flex;gap:0.5rem;align-items:flex-start">
                    <select name="zone_canvas[<?= e($zoneName) ?>]" id="zone_canvas_<?= e($zoneName) ?>"
                            class="form-input" style="flex:1">
                        <option value="">— No canvas assigned —</option>
                        <?php
                        // Show canvases matching this zone_name first
                        if (!empty($canvasesByZone[$zoneName])):
                            ?>
                            <optgroup label="<?= e($zoneLabel) ?> Canvases">
                            <?php foreach ($canvasesByZone[$zoneName] as $zc): ?>
                                <option value="<?= (int)$zc['id'] ?>"
                                    <?= $assignedCanvasId && (int)$zc['id'] === (int)$assignedCanvasId ? 'selected' : '' ?>>
                                    <?= e($zc['title']) ?>
                                    <?= $zc['status'] !== 'published' ? ' [' . e($zc['status']) . ']' : '' ?>
                                </option>
                            <?php endforeach; ?>
                            </optgroup>
                        <?php endif;

                        // Then show all other zone canvases
                        $otherCanvases = array_filter($zoneCanvases ?? [], function($zc) use ($zoneName) {
                            return ($zc['zone_name'] ?? '') !== $zoneName;
                        });
                        if (!empty($otherCanvases)):
                            ?>
                            <optgroup label="Other Canvases">
                            <?php foreach ($otherCanvases as $zc): ?>
                                <option value="<?= (int)$zc['id'] ?>"
                                    <?= $assignedCanvasId && (int)$zc['id'] === (int)$assignedCanvasId ? 'selected' : '' ?>>
                                    <?= e($zc['title']) ?>
                                    <?= $zc['zone_name'] ? ' [' . e($zc['zone_name']) . ']' : '' ?>
                                    <?= $zc['status'] !== 'published' ? ' [' . e($zc['status']) . ']' : '' ?>
                                </option>
                            <?php endforeach; ?>
                            </optgroup>
                    ...
                    <?php if ($assignedCanvas): ?>
                        <a href="<?= url('/admin/editor/' . (int)$assignedCanvas['id'] . '/edit') ?>"
                           class="btn btn-small btn-outline" target="_blank" title="Edit canvas in new tab">
                            Edit ↗
                        </a>
                    <?php endif; ?>
                </div>
            ...
            <div class="form-help" style="margin-top:1rem">
                <a href="<?= url('/admin/site-builder/zones') ?>">Manage zone canvases →</a>
            </div>
