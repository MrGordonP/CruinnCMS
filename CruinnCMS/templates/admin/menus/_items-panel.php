<?php
/**
 * Menu Items Panel — fragment template
 *
 * Rendered standalone (no layout) by MenuController::adminItemsPanel().
 * Injected into the middle panel of the 3-panel menus index via AJAX.
 *
 * Variables: $menu, $items, $pages, $subjects, $routes
 */
?>
<input type="hidden" name="_csrf_token" value="<?= e(\Cruinn\CSRF::getToken()) ?>">
<section class="menu-item-editor" data-menu-id="<?= (int)$menu['id'] ?>">
    ...
    <div class="menu-editor-columns">
        ...
        <?php else: ?>
            ...
            if (!function_exists('renderMenuItemTree')) {
                        ...
                        <li>
                            <label class="menu-source-item <?= in_array($cr['route'], $usedRoutes) ? 'menu-source-used' : '' ?>">
                                ...
                                <?php elseif (($cr['module_status'] ?? 'active') !== 'active'): ?>
                                    <span class="badge badge-warning menu-source-in-use" title="Module not active">inactive</span>
                                <?php endif; ?>
                            </label>
                        </li>
                        <?php endforeach; ?>
