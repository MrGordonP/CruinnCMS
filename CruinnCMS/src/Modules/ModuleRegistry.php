...
class ModuleRegistry
{
    ...
    public static function renderProviderWidget(string $widgetKey, array $settings, array $userContext): string
    {
        ...
        $settings = self::expandWidgetSettings($settings);

        // Virtual widgets are generated by the registry itself (not module manifests).
        $virtual = self::virtualWidgetMap();
        if (isset($virtual[$widgetKey])) {
            return self::renderVirtualWidget($virtual[$widgetKey], $settings, $userContext);
        }
    ...
    private static function collectVirtualWidgetEntries(): array
    {
        $entries = [];
        foreach (self::virtualWidgetMap() as $key => $def) {
    ...
    private static function virtualWidgetMap(): array
    {
        ...
        foreach (self::$modules as $slug => $def) {
            ...
            $sections = (array) ($def['dashboard_sections'] ?? []);
            if (empty($sections)) {
                $sections = (array) ($def['acp_sections'] ?? []);
            }
            ...
            foreach (array_values($sections) as $idx => $section) {
                ...
                $raw = 'section-' . ($section['group'] ?? '') . '-' . $label . '-' . $url;
                $safe = self::normaliseWidgetKeyPart($raw);
                if ($safe === '') {
                    $safe = 'section-link-' . ((int) $idx + 1);
                }
    ...
    private static function renderVirtualWidget(array $def, array $settings, array $userContext): string
    {
        ...
        if ($type === 'quick-link') {
            $label = (string) ($settings['label'] ?? $def['label'] ?? 'Link');
            $url = (string) ($settings['url'] ?? $def['url'] ?? '#');
            $icon = (string) ($settings['icon'] ?? $def['icon'] ?? '🔗');

            $labelEsc = htmlspecialchars($label, ENT_QUOTES, 'UTF-8');
            $urlEsc = htmlspecialchars($url, ENT_QUOTES, 'UTF-8');
            $iconEsc = htmlspecialchars($icon, ENT_QUOTES, 'UTF-8');

            return '<div class="dash-quick-grid">'
                . '<a href="' . $urlEsc . '" class="dash-quick-link">'
    ...
    {
        $expanded = $settings;
        $raw = trim((string) ($settings['settings_json'] ?? ''));
        if ($raw === '') {
            return $expanded;
        }
        ...
        unset($expanded['settings_json']);
        return array_merge($expanded, $decoded);
    }
