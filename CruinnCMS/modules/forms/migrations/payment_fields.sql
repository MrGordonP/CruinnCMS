-- ============================================================
-- Forms Module — Payment Fields Sub-module Schema
-- Requires: payments module installed
-- ============================================================
...
ALTER TABLE `form_submissions`
    ADD COLUMN IF NOT EXISTS `payment_option_id`   INT UNSIGNED NULL                                                                               COMMENT 'Selected price tier at time of submission' AFTER `reviewer_notes`,
    ADD COLUMN IF NOT EXISTS `payment_method`       ENUM('bank_transfer','cash','cheque','stripe') NULL                                            AFTER `payment_option_id`,
    ADD COLUMN IF NOT EXISTS `payment_status`       ENUM('not_required','pending','verified','rejected') NOT NULL DEFAULT 'not_required'            AFTER `payment_method`,
    ADD COLUMN IF NOT EXISTS `payment_verified_by`  INT UNSIGNED NULL                                                                              AFTER `payment_status`,
    ADD COLUMN IF NOT EXISTS `payment_verified_at`  DATETIME NULL                                                                                  AFTER `payment_verified_by`,
    ADD COLUMN IF NOT EXISTS `payment_notes`        TEXT NULL                                                                                      AFTER `payment_verified_at`;
