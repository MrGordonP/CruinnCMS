...
<div class="drivespace">
    ...
    <aside class="fm-sidebar">
        ...
        <nav class="fm-tree">
            <a href="/drivespace" class="fm-tree-item <?= !$currentFolder ? 'active' : '' ?>">
    ...
    <div class="fm-main">
        ...
        <div class="fm-toolbar">
            ...
            <div class="fm-toolbar-actions">
                <a href="/drivespace/upload<?= $currentFolder ? '?folder=' . (int)$currentFolder['id'] : '' ?>" class="btn btn-primary btn-sm">⬆ Upload</a>
            </div>
        </div>

        <!-- Search & filters -->
        <form class="fm-filters" method="get" action="/drivespace">
            ...
            <?php if ($search || $status || $type): ?>
                <a href="/drivespace<?= $currentFolder ? '?folder=' . (int)$currentFolder['id'] : '' ?>" class="btn btn-sm btn-link">Clear</a>
            <?php endif; ?>
        </form>
            ...
            <div class="fm-folder-info">
                ...
                <div class="fm-folder-actions">
                    ...
                    <?php if ($currentFolder['owner_id'] == ($current_user['id'] ?? 0) || ($current_user['role'] ?? '') === 'admin'): ?>
                        <form method="post" action="/drivespace/folders/<?= (int)$currentFolder['id'] ?>/delete" class="inline-form" onsubmit="return confirm('Delete this folder? Contents will be moved to the parent folder.')">
                            <?= csrf_field() ?>
            ...
            <div class="fm-section">
                ...
                <div class="fm-folder-grid">
                    <?php foreach ($subfolders as $subfolder): ?>
                        <a href="/drivespace?folder=<?= (int)$subfolder['id'] ?>" class="fm-folder-card">
        ...
        <div class="fm-section">
            ...
            <?php if (empty($files)): ?>
                <div class="fm-empty">
                    ...
                    <a href="/drivespace/upload<?= $currentFolder ? '?folder=' . (int)$currentFolder['id'] : '' ?>" class="btn btn-primary">Upload a File</a>
                </div>
            <?php else: ?>
                <table class="admin-table fm-file-table">
                    ...
                    <tbody>
                        <?php foreach ($files as $file): ?>
                            <tr class="fm-file-row" onclick="window.location='/drivespace/<?= (int)$file['id'] ?>'">
                                <td class="fm-col-icon"><?= \Cruinn\Services\DocumentService::fileIcon($file['file_ext'] ?? '') ?></td>
                                <td>
                                    <a href="/drivespace/<?= (int)$file['id'] ?>" class="fm-file-title"><?= e($file['title']) ?></a>
                                ...
                                <td class="text-muted"><?= \Cruinn\Services\DocumentService::formatSize((int)($file['file_size'] ?? 0)) ?></td>
...
function renderFolderTree(array $folders, ?int $activeId, int $depth = 0): void {
    foreach ($folders as $folder) {
        ...
        <div class="fm-tree-group" style="padding-left: <?= $depth * 1.2 ?>rem">
            <a href="/drivespace?folder=<?= (int)$folder['id'] ?>" class="fm-tree-item <?= $isActive ? 'active' : '' ?>">
