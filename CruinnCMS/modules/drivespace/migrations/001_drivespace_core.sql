-- Migration 016: File Manager — Google Drive replacement
-- Folders, files with parsing/composing, version history, sharing, publish workflow

-- ── Folders: hierarchical directory structure with visibility ──────
...
-- ── Files: documents with parsed content and publish workflow ──────
...
CREATE TABLE IF NOT EXISTS files (
    ...
    FULLTEXT idx_files_search (title, description),
...
-- ── File versions: full history with parsed snapshots ──────────────
...
-- ── File/folder sharing: granular access control ───────────────────
...
-- ── Publish log: track where files have been published ─────────────
...
-- Set council folder to be visible to council and admin roles
