...
<section class="container forum-page">
    <header class="forum-header">
        ...
        <div class="forum-mod-bar forum-mod-bar--op">
            ...
            <a class="btn btn-xs btn-outline" href="<?= e(rtrim($forumBasePath, '/') . '/thread/' . (int) $thread['id'] . '/edit-title') ?>">Rename</a>
        </div>
        <?php endif; ?>
    </header>
                ...
                <div class="forum-post-main">
                    ...
                    <footer class="forum-post-actions">
                        ...
                        <?php if ((int)$post['author_user_id'] === (int)$currentUserId || !empty($isAdmin)): ?>
                            <a class="btn btn-xs btn-outline" href="<?= e(rtrim($forumBasePath, '/') . '/post/' . (int) $post['id'] . '/edit') ?>">Edit</a>
                            <form method="post" action="<?= e($forumActionBasePath . '/post/' . (int) $post['id'] . '/delete') ?>" class="inline-form"
                        ...
                        <?php if ((int)$post['author_user_id'] !== (int)$currentUserId): ?>
                            <a class="btn btn-xs btn-ghost" href="<?= e(rtrim($forumBasePath, '/') . '/post/' . (int) $post['id'] . '/report') ?>">Report</a>
                        <?php endif; ?>
                    </footer>
                    <?php endif; ?>
                </div>
    ...
    <?php if ($totalPages > 1): ?>
        <div class="pagination">
            <?php if ($page > 1): ?>
                <a class="btn btn-outline btn-small" href="<?= e(rtrim($forumBasePath, '/') . '/thread/' . (int) $thread['id'] . '?page=' . ($page - 1)) ?>">&#8592; Previous</a>
            <?php endif; ?>
            <span class="pagination-info">Page <?= (int)$page ?> of <?= (int)$totalPages ?></span>
            <?php if ($page < $totalPages): ?>
                <a class="btn btn-outline btn-small" href="<?= e(rtrim($forumBasePath, '/') . '/thread/' . (int) $thread['id'] . '?page=' . ($page + 1)) ?>">Next &#8594;</a>
            <?php endif; ?>
        </div>
    <?php endif; ?>
    ...
    <?php if ($canReply): ?>
        <section class="forum-reply-box" id="reply-form">
            <h2>Post Reply</h2>
            <form method="post" action="<?= e($forumActionBasePath . '/thread/' . (int) $thread['id'] . '/reply') ?>">
                <?= csrf_field() ?>
