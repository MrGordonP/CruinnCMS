...
<?php $newThreadUrl = rtrim($forumBasePath, '/') . '/' . ltrim((string) ($category['slug'] ?? ''), '/') . '/new'; ?>
...
<section class="container forum-page">
    <header class="forum-header forum-header-row">
        ...
        <?php if ($canPost): ?>
            <a href="<?= e($newThreadUrl) ?>" class="btn btn-primary">New Thread</a>
        <?php endif; ?>
    </header>
        ...
        <div class="forum-subcategories">
            ...
            <?php foreach ($subcategories as $sub): ?>
                <article class="forum-category-card forum-subcategory-card">
                    <h3><a href="<?= e(rtrim($forumBasePath, '/') . '/' . ltrim((string) ($sub['slug'] ?? ''), '/')) ?>"><?= e($sub['title']) ?></a></h3>
                    <p class="forum-stats">
        ...
        <div class="forum-threads">
            ...
            <?php foreach ($threads as $thread): ?>
                <article class="forum-thread-row<?= !empty($thread['is_pinned']) ? ' is-pinned' : '' ?>">
                    <div>
                        <h2>
                            <a href="<?= e(rtrim($forumBasePath, '/') . '/thread/' . (int) $thread['id']) ?>"><?= e($thread['title']) ?></a>
                        </h2>
                        <p class="forum-meta">
    ...
    <?php if ($totalPages > 1): ?>
        <div class="pagination">
            <?php if ($page > 1): ?>
                <a class="btn btn-outline btn-small" href="<?= e(rtrim($forumBasePath, '/') . '/' . ltrim((string) ($category['slug'] ?? ''), '/') . '?page=' . ($page - 1)) ?>">Previous</a>
            <?php endif; ?>
            <span class="pagination-info">Page <?= (int)$page ?> of <?= (int)$totalPages ?></span>
            <?php if ($page < $totalPages): ?>
                <a class="btn btn-outline btn-small" href="<?= e(rtrim($forumBasePath, '/') . '/' . ltrim((string) ($category['slug'] ?? ''), '/') . '?page=' . ($page + 1)) ?>">Next</a>
            <?php endif; ?>
        </div>
    <?php endif; ?>
</section>
