<div class="organisation-discussion-detail">
    <div class="page-header">
        <div>
            <a href="/organisation/discussions" class="back-link">&larr; All Discussions</a>
            <h1>
    ...
    <div class="moderation-bar">
        <form method="post" action="/organisation/discussions/<?= (int)$discussion['id'] ?>/pin" class="inline-form">
            <?= csrf_field() ?>
            <button type="submit" class="btn btn-sm btn-secondary">
                <?= $discussion['pinned'] ? 'Unpin' : 'Pin' ?>
            </button>
        </form>
        <form method="post" action="/organisation/discussions/<?= (int)$discussion['id'] ?>/lock" class="inline-form">
            <?= csrf_field() ?>
            <button type="submit" class="btn btn-sm btn-secondary">
                <?= $discussion['locked'] ? 'Unlock' : 'Lock' ?>
            </button>
        </form>
        <form method="post" action="/organisation/discussions/<?= (int)$discussion['id'] ?>/delete" class="inline-form"
    ...
    <?php else: ?>
        <div class="reply-form" id="latest">
            <h3>Post a Reply</h3>
            <form method="post" action="/organisation/discussions/<?= (int)$discussion['id'] ?>/reply">
                <?= csrf_field() ?>
